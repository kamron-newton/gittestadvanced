...
sudo /usr/local/bin/testengine -u "#{TestEngineUsername}#" -p "#{TestEnginePassword}#" -H "http://localhost:8080" run project tags People environment=#{TenantCode}# reportFileName #{Release.ReleaseName}#"-"People output=/home/ec2-user/testEngine/reports format pdf /home/ec2-user/testEngine/FusionWebApiAutomationReadyv2.zip
echo "Finished running People test, uploading report to S3 and sending email `date +%T`" >> /home/ec2-user/testEngine/log.txt
aws s3 cp /home/ec2-user/testEngine/reports/#{Release.ReleaseName}#"-"People.pdf s3://#{S3BucketName}#/#{TenantCode}#/TestResultsV2/#{Release.ReleaseName}#/#{Release.ReleaseName}#"-"People.pdf
sudo python3 /home/ec2-user/testEngine/codeDeploy/send_pdf_mail.py People #{Release.ReleaseName}#"-"People

#Run Delta tag and upload results to S3
echo "Queueing Delta test `date +%T`" >> /home/ec2-user/testEngine/log.txt
sudo /usr/local/bin/testengine -u "#{TestEngineUsername}#" -p "#{TestEnginePassword}#" -H "http://localhost:8080" run project tags Delta environment=#{TenantCode}# reportFileName #{Release.ReleaseName}#"-"Delta output=/home/ec2-user/testEngine/reports format pdf /home/ec2-user/testEngine/FusionWebApiAutomationReadyv2.zip
echo "Finished running Delta test, uploading report to S3 and sending email `date +%T`" >> /home/ec2-user/testEngine/log.txt
aws s3 cp /home/ec2-user/testEngine/reports/#{Release.ReleaseName}#"-"Delta.pdf s3://#{S3BucketName}#/#{TenantCode}#/TestResultsV2/#{Release.ReleaseName}#/#{Release.ReleaseName}#"-"Delta.pdf
sudo python3 /home/ec2-user/testEngine/codeDeploy/send_pdf_mail.py Delta #{Release.ReleaseName}#"-"Delta
